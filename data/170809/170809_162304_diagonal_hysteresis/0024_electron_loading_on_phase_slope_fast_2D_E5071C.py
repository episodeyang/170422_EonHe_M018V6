...
x = 0.150
Vtrap_stops = np.arange(0.225, 0.175, -0.005)
...
Ntot = 500
resolution = 0.30E-3
Npts2 = int(np.abs(Vtrap_stops[0]-Vtrap_parks[0])/resolution)
Npts1 = Ntot-Npts2
...
if __name__ == "__main__":
    ...
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        Npts2 = int(np.abs(Vtrap_stops[m]-Vtrap_parks[m])/resolution)
        ...
        while avgs > 0:
            ...
            fastsweep.run((0.00, Vrg_starts[m], Vrg_stops[m]), (Vtrap_parks[m], Vtrap_parks[m], Vtrap_stops[m]),
                          ...
                          sweep_points=(Npts1, Npts2), IFBW=20E3, datafile=dataCache, 
                          measure_downsweep_too=True, 
