...
this_script = r"0028_electron_loading_fast_diagonal_scan_higherVres_E5071C.py"
current_readout_freq = nwa.get_center_frequency()
t0 = time()
expt =  'vary_vtrap_load_high_Vres'

# Vtrap_park = 0.150
timetrace_averages = 4*999
Vtrap_max = 0.500 #To probe the chemical potential of the ensemble
Vtg_bias = -0.25
presweep_refpt_max = 0.470

# The start and stop points of the fast sweep
mu_guess = -0.370
def f_mu(mu):
    return 7.375 * (mu + 0.350)

#Vtrap_park = 0.150
#x = np.arange(0.000, 0.150+0.05, +0.05)
#Vtrap_parks = Vtrap_park * np.ones(len(x))
#Vtrap_stops = 0.140 * np.ones(len(x))
#Vtrap_backs = 0.280 * np.ones(len(x))
...
# x = 0.150
# Vtrap_parks = np.arange(0.300, 0.420+0.005, +0.005)
# Vtrap_park = Vtrap_parks[0]
# Vtrap_stops = 0.280 * np.ones(len(Vtrap_parks))
# Vtrap_backs = 0.480 * np.ones(len(Vtrap_parks))
# Vrg_starts = -1/1.10 * (Vtrap_parks +f_mu(mu_guess)) - x
# Vrg_stops = -1/1.10 * (Vtrap_stops +f_mu(mu_guess)) - x
# Vrg_backs = -1/1.10 * (Vtrap_backs +f_mu(mu_guess)) - x

def generate_sweep_points(Vtrap_park, Vtrap_stops_i, Vtrap_stops_f):
    if Vtrap_park > Vtrap_stops_i and Vtrap_park > Vtrap_stops_f:
        x = 0.150
        dV = -0.01 if Vtrap_stops_f < Vtrap_stops_i else +0.01
        Vtrap_stops = np.arange(Vtrap_stops_i, Vtrap_stops_f, dV)
        Vtrap_parks = Vtrap_park * np.ones(len(Vtrap_stops))
        Vtrap_park = Vtrap_parks[0]
        Vtrap_backs = 0.480 * np.ones(len(Vtrap_stops))
        Vrg_starts = -1/1.10 * (Vtrap_parks +f_mu(mu_guess)) - x
        Vrg_stops = -1/1.10 * (Vtrap_stops +f_mu(mu_guess)) - x
        Vrg_backs = -1/1.10 * (Vtrap_backs +f_mu(mu_guess)) - x
    else:
        print "Vtrap_park must be > Vtrap_stops_i and Vtrap_stops_f"
    return Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs

Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs = \
                    generate_sweep_points(0.400, 0.375, 0.325)
Vtrap_park = Vtrap_parks[0]
...
if do_reload_waveforms:
    ...
    for bnc, ch_voltage_params in zip([fastsweep.bnc1, fastsweep.bnc2],
                                      [(0.00, Vrg_starts[0], Vrg_stops[0], Vrg_backs[0]), (Vtrap_parks[0], Vtrap_parks[0], Vtrap_stops[0], Vtrap_backs[0])]):
...
bnc1_sweeps_up = Vrg_starts[0] > 0
bnc2_sweeps_up = Vtrap_stops[0] > Vtrap_parks[0]
...
if __name__ == "__main__":
    ...
    def load_resonator_not_trap():
        ...
        while temperature > 0.550:
            ...
            sleep(2)
            print '.',
        filament.fire_filament(54, 0.01)
        print "Fired filament!"
        sleep(15.0)
    ...
    if not skip_presweep:
        v2 = np.linspace(0.6, 0.8, 11)
        v1 = np.arange(-2.0, 0.0, 0.25).tolist() + np.arange(0.000, Vtrap_max, 0.005).tolist() + \
             np.arange(Vtrap_max, Vtrap_park, -0.005).tolist() + [Vtrap_park]
        presweep_Vress = np.array(v2.tolist() + list(0.80 * np.ones(len(v1))))
        presweep_Vtraps = np.array(list(-2.0 * np.ones(len(v2))) + v1)
        presweep_Vresguards = np.zeros(len(v1)+len(v2))

        # Make sure to measure the resonator at 0.6 V to see what the Q is there.
        set_voltages(presweep_Vress[0], None, None, None)
        ...
        # Even if we want to skip the presweep, we should probe the state of the ensemble.
        presweep_Vtraps = np.arange(Vtrap_park, Vtrap_max, 0.0025).tolist() \
                          + np.arange(Vtrap_max, Vtrap_park, -0.0025).tolist() + [Vtrap_park]
        presweep_Vresguards = np.zeros(len(presweep_Vtraps))
        presweep_Vress = 0.8 * np.ones(len(presweep_Vtraps))
    ...
    nwa.configure(**nwa_calibration_config)
    if not skip_presweep:
        set_voltages(presweep_Vress[0], presweep_Vtraps[0], presweep_Vresguards[0], Vtg_bias, pinch=-1.00)
    f0, Q = fit_res_gerwin(fpts, mags, span=2E6)
    #change_readout_freq = True
    ...
    if not skip_presweep:
        for k, voltages in tqdm(enumerate(zip(presweep_Vress, presweep_Vtraps, presweep_Vresguards))):
            ...
            if not(k%40) and (k != len(presweep_Vress)-1):
                ...
                nwa.set_format('SLOG')
                nwa.set_trigger_source('BUS')
            elif k == (np.argmin(np.diff(presweep_Vress))+1):
                print "Adjusting NWA center frequency..."
                nwa.configure(start=nwa.get_center_frequency()-0.75E6,
                              stop=nwa.get_center_frequency()+2.00E6,
                              power=calibration_power,
                              ifbw=calibration_ifbw,
                              sweep_points=calibration_sweep_points,
                              averages=calibration_averages)
        ...
        nwa.configure(span=0E6, power=calibration_power, ifbw=ifbw, sweep_points=sweep_points, averages=averages)
        nwa.auto_scale()
        ...
        for k, voltages in tqdm(enumerate(zip(presweep_Vress, presweep_Vtraps, presweep_Vresguards))):
            Vres, Vtrap, Vresguard = voltages[0], voltages[1], voltages[2]
            set_voltages(Vres, Vtrap, Vresguard, None)
            temperature, presweep_phase = take_trace_and_save(averages)

        set_voltages(presweep_Vress[-1], presweep_Vtraps[-1], presweep_Vresguards[-1], Vtg_bias, pinch=-1.00)
        Vress, Vtraps, Vresguards = generate_vpts(Vtrap_parks[0])

    nwa.set_power(power)

    Vtrap_load = np.float(raw_input("Specify Vtrap_load:"))
    Vtrap_stop_i = np.float(raw_input("Specify initial stop trap voltage (<%.3f):"%Vtrap_load))
    Vtrap_stop_f = np.float(raw_input("Specify final stop trap voltage (<%.3f):"%Vtrap_load))

    Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs = \
                        generate_sweep_points(Vtrap_load, Vtrap_stop_i, Vtrap_stop_f)

    # Actual sweep
    # dataCache.new_stack()
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        Vres, Vtrap, Vresguard = voltages[0], voltages[1], voltages[2]
        # set_voltages(None, None, None, None)
        print Vres, Vtrap
        dataCache.post("Vtrap", Vtrap)
        dataCache.post("Vres", Vres)

        k = 0
        N2 = np.int(np.abs(Vtrap_parks[m]-Vtrap_stops[m])/0.5E-3)
        N3 = 400-N2

        avgs = timetrace_averages
        dataCache.post('fast_sweep_points', np.cumsum([N1, N2, N3, N4, N1]))
        while avgs > 0:
            ...
            fastsweep.run((0.00, Vrg_starts[m], Vrg_stops[m], Vrg_backs[m]), (Vtrap_parks[m], Vtrap_parks[m], Vtrap_stops[m], Vtrap_backs[m]),
                          calibration_averages=calibration_averages,
                          calibration_sweep_points=calibration_sweep_points,
                          timetrace_averages=this_trace,
                          sweep_points=(N1, N2, N3, N4), IFBW=20E3, datafile=dataCache,
                          first_time=first_time)
            avgs -= this_trace
            k += 1

    # current_vs = get_voltages()
    # # Sweep down the resonator guard to commence a new sweep for different Vtrap_park
    # for k in np.arange(current_vs[2], 0.00 + 0.01, 0.01):
    #     set_voltages(None, None, k, None)
    print "Finished measurement at Vtrap_park = %.3f V" % (Vtrap_park)
    ...
    # To determine the chemical potential, run the following script...
