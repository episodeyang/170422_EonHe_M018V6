...
this_script = r"0029_test_trapguard_E5071C.py"
expt =  'test_trapguard'
Vtg_bias = 0.00
...
if __name__ == "__main__":
    ...
    v2 = np.arange(-2.0, 0.10, 0.25).tolist() + \
        np.arange(0.10, 0.45, 0.01).tolist() + np.arange(0.45, 0.10, -0.01).tolist() + \
        np.arange(0.10, 0.45, +0.01).tolist() + np.arange(0.45, 0.10, -0.01).tolist() + \
        np.arange(0.10, 0.45, +0.01).tolist() + np.arange(0.45, 0.10, -0.01).tolist()
    singlesweeplen = len(np.arange(0.10, 0.45, +0.01).tolist() + np.arange(0.45, 0.10, -0.01).tolist())
    v3 = np.repeat(np.array([0, -0.5, 0]), singlesweeplen)

    Vress = v1 + list(0.80 * np.ones(len(v2)))
    Vtraps = np.array(list(-2.0 * np.ones(len(v1))) + v2)
    Vresguards = np.zeros(len(v1) + len(v2))
    Vtrapguards = np.zeros(len(v1) + len(v2))
    Vtrapguards[-len(v3):] = v3
    ...
    set_voltages(Vress[0], Vtraps[0], Vresguards[0], Vtrapguards[0], pinch=-1.00)
    f0, Q = fit_res_gerwin(fpts, mags, span=2E6)
    ...
    if change_readout_freq or np.abs(current_readout_freq - f0) > 150E3:
        ...
        nwa.set_center_frequency(f0+0.25E6)
        print "Drive frequency set to new value: Delta f = %.3f MHz" % ((f0 - 6.40511E9) / 1E6)
    p1, p2, p3, p4, constant_Vpinch = get_voltages()

    # Actual sweep
    nwa.set_span(1.5E6)
    sleep(180)
    for k, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards, Vtrapguards))):
        Vres, Vtrap, Vresguard, Vtrapguard = voltages[0], voltages[1], voltages[2], voltages[3]
        print Vres, Vtrap, Vresguard
        set_voltages(Vres, Vtrap, Vresguard, Vtrapguard)
        ...
        active_electrodes[3] = Vtrapguard if Vtrapguards[k] == Vtrapguards[k - 1] else np.nan
