...
#res = seekat
t0 = time()
expt = 'vary_trap_loading_fast_20mK'
...
if __name__ == "__main__":
    ...
    def set_voltages(res, trap, res_guard, trap_guard, pinch=None, verbose=True):
        ...
        if trap is not None:
            fastsweep.bnc2.set_offset(trap) #change_sweep_bounds(fastsweep.bnc2, trap, trap + 0.10)
            # yoko1.set_volt(trap)
            #seekat.set_voltage(2, trap, verbose=verbose)
        if res_guard is not None:
            fastsweep.change_sweep_bounds(fastsweep.bnc1, res_guard, res_guard-0.50)
        ...
        dataCache.post("voltage_log", np.array([time(),
                                                ...
                                                fastsweep.get_idle_value(fastsweep.bnc1, sweep_up=True),
    ...
    def get_voltages(active_electrodes=[np.nan]*5):
        ...
        for k in np.where(np.isnan(active_electrodes))[0]:
            if k == 1:
                # Trap electrode
                ret[1] = fastsweep.bnc2.get_offset() #fastsweep.get_idle_value(fastsweep.bnc2, sweep_up=True)
            elif k == 2:
                # Resonator guard electrode
                ret[2] = fastsweep.get_idle_value(fastsweep.bnc1, sweep_up=True)
    ...
    # Here we upload the waveform in advance before we begin, because we don't want to get a spike in voltage when this
    # happens right before the sweep starts. At that point the electrons loaded in the trap may escape.
    sweep_up_1 = True if Vrg_ac_stop > Vrg_ac_start else False
    sweep_up_2 = True
    fastsweep.bnc1.set_function('USER')
    fastsweep.bnc2.set_function('DC')
    for sweep_up, bnc in zip([sweep_up_1, sweep_up_2], [fastsweep.bnc1]):
        if sweep_up:
        ...
        if sweep_up:
            fastsweep.change_sweep_bounds(bnc, Vrg_ac_start, Vrg_ac_stop)
        else:
            fastsweep.change_sweep_bounds(bnc, Vrg_ac_start, Vrg_ac_stop)
    ...
    def generate_vpts(vtrap_park):
        # Actual sweep
        Vtraps = np.arange(vtrap_park, 0.1800-0.0020, -0.0020)
    ...
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        dataCache.post("Vtrap", Vtrap)
        dataCache.post("Vres", Vres)
        # dataCache.post("Vrg", Vresguard)

        k = 0
        avgs = timetrace_averages
        while avgs > 0:
            this_trace = 999 if avgs > 999 else avgs
            print "Averaging at Vtrap = %.3f V : %d/%d averages"%(Vtrap, timetrace_averages-this_trace, timetrace_averages)
            dataCache.post('sweep_point', m)
            dataCache.post('timetrace_averages', this_trace)
            send_waveform = True if k == m == 0 else False
            fastsweep.run(Vrg_ac_start, Vrg_ac_stop, 0, 0,
                          calibration_averages=calibration_averages,
                          calibration_sweep_points=calibration_sweep_points,
                          timetrace_averages=this_trace,
                          sweep_points=700, IFBW=20E3, datafile=dataCache, first_time=False)
            avgs -= this_trace
            k += 1
