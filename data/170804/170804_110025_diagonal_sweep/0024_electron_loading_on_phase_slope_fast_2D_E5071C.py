...
if __name__ == "__main__":
    ...
    # Presweep
    v1 = np.arange(-2.0, 0.0, 0.25).tolist() + np.arange(0.000, 0.280, 0.0025).tolist() + \
         np.arange(0.280, presweep_refpt_max, -0.0025).tolist() + [presweep_refpt_max]
    ...
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        Ntot = 500
        Npts1 = int(np.abs(Vtrap_stops[m]-Vtrap_parks[m])/0.25E-3)
        Npts2 = Ntot-Npts1 #int(np.abs(Vrg_starts[m])/1.5E-3)
