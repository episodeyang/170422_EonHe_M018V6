...
if __name__ == "__main__":
    ...
    def set_voltages(res, trap, res_guard, trap_guard, pinch=None, verbose=True):
        ...
        dataCache.post("voltage_log", np.array([time(),
                                                ...
                                                fastsweep.get_idle_value(fastsweep.bnc2, sweep_up=True),
                                                fastsweep.get_idle_value(fastsweep.bnc1, sweep_up=False),
    ...
    def get_voltages(active_electrodes=[np.nan]*5):
        ...
        for k in np.where(np.isnan(active_electrodes))[0]:
            if k == 1:
                # Trap electrode
                ret[1] = fastsweep.get_idle_value(fastsweep.bnc2, sweep_up=True)
            elif k == 2:
                # Resonator guard electrode
                ret[2] = fastsweep.get_idle_value(fastsweep.bnc1, sweep_up=False)
            else:
    ...
    Vtrap_stops = 0.320 * np.ones(len(x))
