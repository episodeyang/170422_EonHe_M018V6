...
t0 = time()
expt = 'load_and_diagonal_equal_dv'

if __name__ == "__main__":
    today = strftime("%y%m%d")
    now = strftime("%H%M%S")
    expt_path = os.path.join(r'C:\Users\slab\Desktop\Gerwin\data', today, "%s_%s" % (now, expt))
    ...
    Vtrap_stops = 0.265 * np.ones(len(x))
    Vrg_starts = -1/1.15 * (Vtrap_park - (f_mu(mu_guess)-0.120)) - x
    Vrg_stops = -1/1.15 * (Vtrap_stops -(f_mu(mu_guess)-0.120)) - x

    # Presweep
    v1 = np.arange(-2.0, 0.0, 0.25).tolist() + np.arange(0.000, 0.250, 0.0025).tolist() + \
         np.arange(0.250, presweep_refpt_max, -0.0025).tolist() + [presweep_refpt_max]
    ...
    dataCache.post('presweep_Vres', presweep_Vresguards)
    dataCache.post('presweep_Vresguard', presweep_Vress)
    ...
    if not((target_deltaf-0.15E6) < np.abs(f0-6.40511E9) < (target_deltaf+0.05E6)) or Q < target_Q:
        unload()
        load_electrons = True
        ...
        if load_electrons:
            ...
            while Q_pre_meas < target_Q:
                ...
                while (abs_deltaf > target_deltaf) and (tries < 15):
                    tries += 1
                    if (abs_deltaf - target_deltaf) < 0.30E6 and tries == 1:
                        unload_voltage = -0.15
    ...
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        Ntot = 700
        Npts1 = 150#int(np.abs(Vrg_starts[m])/0.001)
        Npts2 = 250#Ntot-Npts1
