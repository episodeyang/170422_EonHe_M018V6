...
import quicktimetrace_3D as fastsweep

this_script = r"0024_electron_loading_on_phase_slope_fast_2D_E5071C.py"
current_readout_freq = nwa.get_center_frequency()
t0 = time()
expt =  'diagonal_sweep'

# Vtrap_park = 0.150
timetrace_averages = 999
presweep_refpt_max = Vtrap_park
Vtrap_max = 0.330 #To probe the chemical potential of the ensemble
...
x = np.arange(0.1500, -0.0075, -0.0075)
Vtrap_parks = Vtrap_park * np.ones(len(x))
Vtrap_stops = 0.190 * np.ones(len(x))
Vrg_starts = -1/1.15 * (Vtrap_park - (f_mu(mu_guess)-0.120)) - x
Vrg_stops = -1/1.15 * (Vtrap_stops -(f_mu(mu_guess)-0.120)) - x

Ntot = 500
Npts2 = int(np.abs(Vtrap_stops[0]-Vtrap_parks[0])/0.30E-3)
Npts1 = Ntot-Npts2
if Npts2 > Ntot:
    raise ValueError("Resolution in 2nd part of trace too high. Decrease Npts2, or increase Ntot!")
...
for bnc, ch_voltage_params in zip([fastsweep.bnc1, fastsweep.bnc2], 
                                  [(0.00, Vrg_starts[0], Vrg_stops[0]), (Vtrap_parks[0], Vtrap_parks[0], Vtrap_stops[0])]):
    fastsweep.setup_waveforms(bnc, ch_voltage_params, (Npts1, Npts2), sweep_time=100E-3)
...
if __name__ == "__main__":
    ...
    if not skip_presweep:
        v1 = np.arange(-2.0, 0.0, 0.25).tolist() + np.arange(0.000, Vtrap_max, 0.0025).tolist() + \
             np.arange(Vtrap_max, presweep_refpt_max, -0.0025).tolist() + [presweep_refpt_max]
        presweep_Vress = 0.60 * np.ones(len(v1))
        presweep_Vtraps = np.array(v1)
        presweep_Vresguards = np.zeros(len(v1))
        ...
        presweep_Vress = 0.6 * np.ones(len(presweep_Vtraps))        
    ...
    if not skip_presweep:
        ...
        nwa.configure(span=0E6, ifbw=ifbw, sweep_points=sweep_points, averages=averages)
        nwa.auto_scale()
        ...
        set_voltages(presweep_Vress[-1], presweep_Vtraps[-1], presweep_Vresguards[-1], 0.000, pinch=-1.00)
    ...
    for m, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        Ntot = 600
        Npts2 = int(np.abs(Vtrap_stops[m]-Vtrap_parks[m])/0.25E-3)
        Npts1 = Ntot-Npts2 #int(np.abs(Vrg_starts[m])/1.5E-3)
        print Npts1, Npts2
        dataCache.post('fast_sweep_points', np.array([Npts1, Npts2]))
        while avgs > 0:
            first_time = True if (k == 0) else False
            this_trace = 999 if avgs > 999 else avgs
            print "Averaging at Vtrap = %.3f V : %d/%d averages"%(Vtrap, timetrace_averages-avgs, timetrace_averages)
            ...
            fastsweep.run((0.00, Vrg_starts[m], Vrg_stops[m]), (Vtrap_parks[m], Vtrap_parks[m], Vtrap_stops[m]),
                          calibration_averages=calibration_averages,
                          calibration_sweep_points=calibration_sweep_points,
                          timetrace_averages=this_trace,
                          sweep_points=(Npts1, Npts2), IFBW=20E3, datafile=dataCache, first_time=first_time)
            avgs -= this_trace
            k += 1
