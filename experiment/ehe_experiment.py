...
class eHeExperiment():
    def __init__(self, config_file_path, data_directory=None, verbose=True):
        ...
        with open(config_file_path, 'r') as config_file:
            ...
            self.config = utils.Struct(__default=None, **yaml.load(config_file))
            config_file.seek(0)
        ...
        self.dash = visdom_helper.Dashboard(env, server="http://192.168.14.15", port=8097)
    ...
    def sweep(self, sweep_generator, save=True):
        ...
        for params in tqdm(list(sweep_generator)):
            ...
            self.set_trap_guard_voltage(params['Vtrap_guard'])
            self.set_res_guard_voltage(params['Vres_guard'])
            self.set_pinch_voltage(params['Vpinch'])

            if save:
                self.take_trace_and_save()
    ...
    def plot_sweep_params(self, data, index=None, title="sweep_parameters", blocking=True):
        ...
        plt.legend(edgecolor='none', framealpha=1, frameon=False)
    ...
    def get_res_guard_voltage(self):
        return self.seekat.get_voltage(self.config.seekat.res_guard_channel)

    def set_res_guard_voltage(self, volt):
        self.seekat.set_voltage(self.config.seekat.res_guard_channel, volt)

    def get_trap_guard_voltage(self):
        return self.seekat.get_voltage(self.config.seekat.trap_guard_channel)

    def set_trap_guard_voltage(self, volt):
        self.seekat.set_voltage(self.config.seekat.trap_guard_channel, volt)
    ...
    def take_trace_and_save(self):
        ...
        self.dataCache.post('Vres_guard', self.get_res_guard_voltage())
        self.dataCache.post('Vtrap_guard', self.get_trap_guard_voltage())
        ...
        vress, vtraps, vres_guards, vtrap_guards, vpinches, = \
            self.dataCache.get('Vres'), self.dataCache.get('Vtrap'), self.dataCache.get(
            'Vres_guard'), self.dataCache.get('Vtrap_guard'), self.dataCache.get('Vpinch')
        self.dash.plot('bias-electrodes', 'line',
                       X=np.arange(len(vress)),
                       Y=np.column_stack((vress, vtraps, vres_guards, vtrap_guards, vpinches)),
                       opts=dict(legend=['Vres', 'Vtrap', 'Vres_guard', 'Vtrap_guard', 'Vpinch'],)
                       )

        self.dash.append('resonance frequency', 'line', X=np.array(vress.shape[:1]), Y=np.array([self.peak_tracker.f0]),
                         opts=dict(legend=['f0s']))
        return temperature
    ...
    def fire_and_load(self):
        ...
        self.set_res_guard_voltage(self.config.experiment.loading_Vres_guard)
        self.set_trap_guard_voltage(self.config.experiment.loading_Vtrap_guard)
    ...
    def unload(self):
        unload_params = self.config.experiment.unload
        for p in unload_params:
            sweep_type, = p.keys()
            sweep_params = p[sweep_type]
            sweep_gen = lambda: sweep_types.__dict__[sweep_type](**sweep_params)
            self.sweep(sweep_gen(), False)

    def load_electrons_till(self):
        if self.config.experiment.loading_criteria:
            loading_criteria = self.config.experiment.loading_criteria
            base_Q = loading_criteria.Q
            base_df = loading_criteria.df
            delay_between_trials = loading_criteria.delay_between_trials
            Q, df = 0, 0
            print("Q is {}, df is {}, f0 is {}".format(Q, df, self.peak_tracker.f0))
            while not (Q > base_Q and df > base_df):
                if self.config.experiment.unload:
                    print('unloading electrons')
                    self.unload()
                self.fire_and_load()
                print('firing and loading')

                self.set_res_voltage(loading_criteria.Vres)
                self.set_trap_voltage(loading_criteria.Vtrap)
                self.set_res_guard_voltage(loading_criteria.Vres_guard)
                self.set_trap_guard_voltage(loading_criteria.Vtrap_guard)
                self.set_pinch_voltage(loading_criteria.Vpinch)

                self.take_trace_and_save()
                df = np.abs(self.peak_tracker.f0 - self.config.experiment.loading_criteria.f0)
                Q = self.peak_tracker.f0 / (2 * self.peak_tracker.fw)
                print("Q is {}, df is {}, f0 is {}".format(Q, df, self.peak_tracker.f0))
                if delay_between_trials:
