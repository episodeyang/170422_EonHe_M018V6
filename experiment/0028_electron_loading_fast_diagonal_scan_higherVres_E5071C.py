...
t0 = time()
expt =  'make_2d_diagonal_sweep'

# Vtrap_park = 0.150
timetrace_averages = 6*999
Vtrap_max = 0.450 #To probe the chemical potential of the ensemble
Vtg_bias = 0.00
...
def generate_sweep_points(Vtrap_park):
    x = np.arange(0.150, 0.050, -0.025)
    Vtrap_parks = Vtrap_park * np.ones(len(x))
    Vtrap_stops = 0.280 * np.ones(len(x))
    Vtrap_backs = 0.480 * np.ones(len(x))
    Vrg_starts = -1/1.10 * (Vtrap_parks +f_mu(mu_guess)) - x
    Vrg_stops = -1/1.10 * (Vtrap_stops +f_mu(mu_guess)) - x
    Vrg_backs = -1/1.10 * (Vtrap_backs +f_mu(mu_guess)) - x
    return Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs

# def generate_sweep_points(Vtrap_parks_i, Vtrap_parks_f):
#     x = 0.150
#     dV = -0.01 if Vtrap_parks_f < Vtrap_parks_i else +0.01
#     Vtrap_parks = np.arange(Vtrap_parks_i, Vtrap_parks_f, dV)
#     Vtrap_stops = 0.280 * np.ones(len(Vtrap_parks))
#     Vtrap_backs = 0.480 * np.ones(len(Vtrap_parks))
#     Vrg_starts = -1/1.10 * (Vtrap_parks +f_mu(mu_guess)) - x
#     Vrg_stops = -1/1.10 * (Vtrap_stops +f_mu(mu_guess)) - x
...
Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs = generate_sweep_points(0.300)
Vtrap_park = Vtrap_parks[0]
...
if __name__ == "__main__":
    ...
    def set_voltages(res, trap, res_guard, trap_guard, pinch=None, verbose=True):
        ...
        if trap is not None:
            if bnc2_sweeps_up:
                ...
                # trap - Vi / (Vf - Vi) = (Vtrap_parks[0] - Vtrap_stops[0]) / (Vtrap_backs[0] - Vtrap_stops[0])
                #alpha = (Vtrap_parks[0] - Vtrap_stops[0]) / (Vtrap_backs[0] - Vtrap_stops[0])
                #beta = 0.05
                #Vi = alpha*beta + trap
                #Vf = Vi-beta
                fastsweep.change_sweep_bounds(fastsweep.bnc2, trap, trap-0.05)
    ...
    def load_resonator_not_trap():
        ...
        while temperature > 0.550:
            ...
            sleep(2)
            print '.',
        filament.fire_filament(59, 0.01)
        print "Fired filament!"
        sleep(15.0)
    ...
    Vtrap_park = np.float(raw_input("Specify trap load voltage:"))

    Vtrap_parks, Vtrap_stops, Vtrap_backs, Vrg_starts, Vrg_stops, Vrg_backs = generate_sweep_points(Vtrap_park)
