...
from setup_instruments import fridge, yoko1, seekat, nwa, filament
...
if __name__ == "__main__":
    ...
    def set_voltages(res, trap, res_guard, trap_guard, pinch=None, verbose=True):
        ...
        if trap is not None:
            fastsweep.change_sweep_bounds(fastsweep.bnc2, trap, trap-0.10) #yoko1.set_volt(trap)
            #seekat.set_voltage(2, trap, verbose=verbose)
        if res_guard is not None:
            pass#yoko2.set_volt(res_guard)
        ...
        dataCache.post("voltage_log", np.array([time(),
                                                seekat.get_voltage(1), fastsweep.get_idle_value(fastsweep.bnc2, sweep_up=False),
                                                0.00, seekat.get_voltage(4),
    ...
    def get_voltages(active_electrodes=[np.nan]*5):
        ...
        for k in np.where(np.isnan(active_electrodes))[0]:
            if k == 1:
                # Trap electrode
                ret[1] = fastsweep.get_idle_value(fastsweep.bnc2, sweep_up=False)#yoko1.get_volt()
            elif k == 2:
    ...
    # Presweep
    v1 = np.arange(0.000, presweep_refpt_max, 0.005).tolist() + [presweep_refpt_max]
    ...
    if expt == 'determine_mu':
        def generate_vpts(vtrap_park, vtrap_unload_stop, vrg_isolate):
            # Actual sweep
            Vtraps = np.arange(vtrap_park, 0.320, 0.005).tolist()
            for Stop in np.arange(0.320, 0.400, 0.005):
                Vtraps += np.arange(Stop, 0.200, -0.005).tolist() + np.arange(0.200, Stop, +0.005).tolist()
            Vtraps += np.arange(Stop, 1.0, 0.005).tolist()
    ...
    if 1:#np.abs(f0-6.40511E9) < (target_deltaf-0.30E6) or Q < target_Q:
        unload()
        load_electrons = True
