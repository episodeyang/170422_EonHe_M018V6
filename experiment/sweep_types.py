...
def sweep_2d_trap_first(Vguard, Vres_start, Vres_end, dVres, dVtrap, offset_limit=0.5, bi_dir=True, reverse=False, **rest):
    dVres = abs(dVres) if Vres_start < Vres_end else -abs(dVres)
    Vress = np.arange(Vres_start, Vres_end + dVres, dVres)
    for Vres in Vress:
        ...
        if not bi_dir:
            continue
        for data in trap_voltage_sweep(Vres, Vguard, Vtrap_end, Vtrap_start, dVtrap, **rest):
            yield data