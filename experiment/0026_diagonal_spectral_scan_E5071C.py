...
if __name__ == "__main__":
    ...
    v1 = np.arange(-2.0, 0.0, 0.25).tolist() + np.arange(0.000, Vtrap_max, 0.0025).tolist() + \
         np.arange(Vtrap_max, Vtrap_parks, -0.0025).tolist() + [Vtrap_parks]
    v2 = fastsweep.construct_voltage_trace(Vtrap_parks, Vtrap_parks, Vtrap_stops, Vtrap_backs,
                                           (N1, N2, N3, N4), include_offset=True)
    v3 = fastsweep.construct_voltage_trace(0, Vrg_starts, Vrg_stops, Vrg_backs,
                                           (N1, N2, N3, N4), include_offset=True)

    Vress = 0.60 * np.ones(len(v1)+len(v2))
    Vtraps = np.array(v1 + v2.tolist())
    Vresguards = np.array(np.zeros(len(v1)).tolist() + v3.tolist())
    ...
    for k, voltages in tqdm(enumerate(zip(Vress, Vtraps, Vresguards))):
        ...
        take_trace_and_save(calibration_averages, active_electrodes=active_electrodes)
