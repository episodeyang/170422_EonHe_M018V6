...
this_script = r"0025_electron_loading_on_phase_slope_fast_also_up_E5071C.py"
current_readout_freq = nwa.get_center_frequency()
t0 = time()
expt =  'vary_vtrap_load'

# Vtrap_park = 0.150
timetrace_averages = 4*999
Vtrap_max = 0.290 #To probe the chemical potential of the ensemble
Vtg_bias = 0.00
presweep_refpt_max = Vtrap_max
...
x = 0.200
Vtrap_parks = np.arange(0.250, 0.200-0.01, -0.01)
Vtrap_park = Vtrap_parks[0]
Vtrap_stops = 0.180 * np.ones(len(Vtrap_parks))
Vtrap_backs = 0.280 * np.ones(len(Vtrap_parks))
Vrg_starts = -1/1.15 * (Vtrap_parks - (f_mu(mu_guess)-0.120)) - x
Vrg_stops = -1/1.15 * (Vtrap_stops -(f_mu(mu_guess)-0.120)) - x
Vrg_backs = -1/1.15 * (Vtrap_backs -(f_mu(mu_guess)-0.120)) - x
