...
this_script = r"0022_electron_loading_on_phase_slope_2D_E5071C.py"
...
if __name__ == "__main__":
    ...
    v2 = np.arange(0.000, -0.550-0.005, -0.005).tolist()
    presweep_Vress = 0.60 * np.ones(len(v1)+len(v2))
    presweep_Vtraps = np.array(v1 + list(Vtrap_park * np.ones(len(v2))))
    presweep_Vresguards = np.array(np.zeros(len(v1)).tolist() + v2)

    presweep_len = len(v1)+len(v2)
    presweep_refpt = presweep_Vtraps[np.argmin(np.abs(presweep_Vtraps - 0.150))]
    presweep_fthreshold = None # Positive threshold is measured BELOW the frequency shift at the ref voltage
    ...
    def generate_vpts(vtrap_park, vtrap_unload_stop, vrg_isolate):
        ...
        dVrg = +0.01
        dVtrap = 1E-3
        Vrgs = np.arange(-0.550, -0.300+dVrg, dVrg)
        for k, Vrg in enumerate(Vrgs):
            iso = 1/10. * (vtrap_park - 1.15*Vrg - 0.320)
            Vtrap = np.array(np.arange(vtrap_park, 0.08, -dVtrap).tolist() +
                             np.arange(0.08, vtrap_park+dVtrap, +dVtrap).tolist())
            Vrgsweep = 1/1.15 * (Vtrap - 0.320 - 10*iso)
            Vresguards = Vrgsweep if k==0 else np.append(Vresguards, Vrgsweep)
            Vtraps = Vtrap if k==0 else np.append(Vtraps, Vtrap)
    ...
    plt.plot(sweep_Vresguards, sweep_Vtraps, '.-', ms=3, color="red", markeredgecolor="none")
